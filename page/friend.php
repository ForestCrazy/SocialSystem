...
if (!isset($_SESSION["user_id"]) OR !isset($_SESSION["username"])) {
    ...
    $sql_friend_pending = 'SELECT * FROM friendrelation WHERE user_id_1 = "' . $_SESSION['user_id'] . '" AND AreFriend = "False"';
    $res_friend_pending = mysqli_query($connect, $sql_friend_pending);
    
    $sql_friend_request = 'SELECT * FROM friendrelation WHERE user_id_2 = "' . $_SESSION['user_id'] . '" AND AreFriend = "False"';
