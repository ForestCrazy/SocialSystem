...
if (!isset($_SESSION["user_id"]) OR !isset($_SESSION["username"]) OR !isset($_SESSION["level"])) {
    ...
    if (isset($_GET['user_id'])) {
        ...
        $sql_feed_post_list = 'SELECT * FROM (SELECT * FROM account LEFT JOIN (SELECT * FROM friendrelation WHERE user_id_1 = "' . $_SESSION['user_id'] . '" OR user_id_2 = "' . $_SESSION['user_id'] . '") AS friendrelation ON account.user_id = friendrelation.user_id_1 OR account.user_id = friendrelation.user_id_2 WHERE AreFriend IS NOT NULL) AS user_list INNER JOIN post ON user_list.user_id = post.user_id ORDER BY post.post_id';
    }
